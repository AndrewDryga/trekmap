defmodule Trekmap.Bots.FleetCommander2 do
  use GenServer
  ...
  def init([]) do
    Logger.info("[FleetCommander2] Waiting for commands")
  ...
  def handle_call(:get_ships_on_mission, _from, %{on_mission: true} = state) do
    {:reply, [Fleet.kehra_fleet_id()], state}
  end
  ...
  def handle_cast(:continue_missions, %{session: session, on_mission: false} = state) do
    ...
    Logger.info("[FleetCommander2] Starting missions using fleet: #{inspect(fleet.id)}")
  ...
  def handle_info({:continue_mission, %{state: :at_dock}}, %{session: session} = state) do
    Logger.info("[FleetCommander2] Fleet is at docks")
  ...
  def handle_info({:continue_mission, %{state: :fighting}}, %{session: session} = state) do
    Logger.info("[FleetCommander2] Fleet is fighting")

    :timer.sleep(:timer.seconds(5))
    fleet = reload_kehra_fleet(session)
    send(self(), {:continue_mission, fleet})
  ...
  def handle_info({:continue_mission, %{state: :mining}}, %{session: session} = state) do
    fleet = reload_kehra_fleet(session)
    Logger.info("[FleetCommander2] Fleet is mining, hiding at #{inspect(fleet.coords)}")
  ...
  def handle_info({:continue_mission, %{state: fleet_state} = fleet}, %{session: session} = state)
    ...
    Logger.info(
      "[FleetCommander2] Fleet is #{inspect(fleet_state)}, " <>
  ...
  def handle_info(
    ...
    Logger.info("[FleetCommander2] Fleet hull is damaged #{hull_health}, recalling")
  ...
  #   Logger.info("[FleetCommander2] Fleet shield is damaged #{shield_health}, waiting")
  #
  ...
  def handle_info(
    ...
    Logger.info("[FleetCommander2] Fleet is full, recalling")
  ...
  def handle_info({:continue_mission, fleet}, %{on_mission: true} = state) do
    Logger.debug("[FleetCommander2] Continue, #{inspect(fleet, pretty: true)}")
      ...
      |> Enum.reduce_while({fleet, :next_system}, fn target, {fleet, _next_step} ->
        if distance(target.coords, fleet.coords) < 50 do
          ...
          case Trekmap.Me.attack_miner(fleet, target, session) do
            ...
            other ->
              Logger.warn("[FleetCommander2] Cant kill #{inspect({target, other}, pretty: true)}")
          ...
          case Trekmap.Me.fly_to_coords(fleet, target.coords, session) do
            ...
            other ->
              Logger.warn(
                "[FleetCommander2] Cant approach #{inspect({target, other}, pretty: true)}"
              )
    ...
    case continue do
      :next_system ->
        ...
        if nearby_system_with_targets do
          ...
          Logger.info("[FleetCommander2] Warping to next system #{system.name} (#{system.id})")
          ...
          Logger.info("[FleetCommander2] No more targets in any systems")
      ...
      :recall ->
        Logger.info("[FleetCommander2] Shield is active, delaying missions by 1 hour")
  ...
  def reload_kehra_fleet(session) do
  ...
  def get_initial_fleet(session) do
    ...
    cond do
      deployed_fleets == %{} ->
        Logger.info("[FleetCommander2] No fleets deployed, picking kehra")
        fleet = %Fleet{id: Fleet.kehra_fleet_id(), system_id: session.home_system_id}
  ...
  defp can_kill?(miner, fleet) do
    if miner.strength do
      miner.strength * 1.2 < fleet.strength
    else
      false
    end
  end
  ...
  defp recall_fleet(session) do
    ...
    if kehra = Map.get(deployed_fleets, to_string(Fleet.kehra_fleet_id())) do
      Logger.info("[FleetCommander2] Recalling fleet")
      ...
      |> case do
        {:ok, fleet} ->
          Logger.info(
            "[FleetCommander2] Fleet recalled, current state: #{to_string(fleet.state)}"
          )
